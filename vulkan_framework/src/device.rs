...
pub struct Device {
    data: Box<DeviceData>,
    instance: Weak<Mutex<crate::instance::Instance>>,
    extensions: DeviceExtensions,
    device: ash::Device,
}

impl InstanceOwned for Device {
    fn get_parent_instance(&self) -> Weak<Mutex<crate::instance::Instance>> {
        self.instance.clone()
    }
}
            ...
            Some(instance_mutex) => {
                match instance_mutex.lock() {
                    ...
                    Err(err) => {
                        ...
                        {
                            println!("Cannot acquire Instance mutex: {}.", err);
...
impl Device {
     ...
     *
     * This method can be called in every point in time, but it will not succeed if the Arc<Mutex<Instance>>
     ...
     */
    pub fn new(
        instance_weak_ptr: Weak<Mutex<crate::instance::Instance>>,

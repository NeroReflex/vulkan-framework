...
impl PrivateShaderTrait for CallableShader {
...
impl CallableShader {
    pub fn new<'a, 'b, 'c>(
        ...
        match unsafe {
            ...
            Err(err) => Err(VulkanError::Vulkan(err.as_raw(), Some(format!("Error creating the callable shader set: {}", err.to_string()))))
        }
    }
}

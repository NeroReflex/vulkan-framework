...
use crate::{image::ImageTrait, device::DeviceOwned, swapchain::{SwapchainKHROwned, SwapchainKHR}, prelude::{VulkanResult, VulkanError}};
        ...
        match swapchain.get_parent_device().ash_ext_swapchain_khr() {
            Option::Some(ext) => {
                match unsafe { ext.get_swapchain_images(swapchain.ash_handle()) } {
                    Ok(images) => {
                        Ok(
                            images.into_iter().map(|swapchain_image| 
                                Arc::new(
                                    Self {

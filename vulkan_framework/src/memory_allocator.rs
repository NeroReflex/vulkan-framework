...
impl AllocationResult {
    pub fn new(requested_size: u64, requested_alignment: u64, resulting_address: u64, allocation_start: u64, allocation_end: u64) -> Self {
        Self {
            requested_size,
            requested_alignment,
            resulting_address,
    ...
    ) -> Option<AllocationResult>
    {
        match self.allocated_size.lock() {
            Ok(mut allocated_size_guard) => {
                ...
                let mut padding_to_respect_aligment = ((allocated_number_of_aligned_blocks + 1) * alignment) - (allocated_number_of_aligned_blocks * alignment);
                ...
                match self.total_size >= allocation_end {
                    true => {
                        *allocated_size_guard += allocation_end - allocation_start;
                        Some(AllocationResult::new(size, alignment, allocation_start + padding_to_respect_aligment, allocation_start, allocation_end))
                    }
                    false => {
                        None
                    }
        ...
        match self.allocated_size.lock() {
            Ok(mut allocated_size_guard) => {
                ...
                if allocation_to_undo.allocation_end == allocation_start {
                    *allocated_size_guard -= allocation_to_undo.allocation_end - allocation_to_undo.allocation_start;
                } else {
                    #[cfg(debug_assertions)]
                    {

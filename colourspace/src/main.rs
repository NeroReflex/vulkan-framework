...
mod lan;
use lan::{spawn_worker, MeasureRequest};
    ...
    let elapsed = t0.elapsed().as_secs_f32();
        ...
        if let Some((tx, rx)) = worker.as_ref() {
            ...
            if elapsed.fract() < 0.02 {
                ...
                let _ = tx.send(MeasureRequest { red: r as u8, green: g as u8, blue: b as u8 });
            }

            // non-blocking receive of any available responses
            loop {
                match rx.try_recv() {
                    Ok(Ok(res)) => println!("Measured result: x={:?} y={:?} y_lum={:?}", res.x, res.y, res.y_lum),
                    Ok(Err(e)) => eprintln!("Measurement error: {}", e),
                    Err(TryRecvError::Empty) => break,
                    Err(TryRecvError::Disconnected) => {
                        eprintln!("ColourSpace worker disconnected");
                        break;
                    }
                }
            }
